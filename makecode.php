...
if ( isset($_GET['code']) ) {
    ...
    } else if ( intval($code[0]) < 0 || intval($code[0]) > 9 || 
                intval($code[1]) < 0 || intval($code[1]) > 9 ||
                intval($code[2]) < 0 || intval($code[2]) > 9 ||
                intval($code[3]) < 0 || intval($code[3]) > 9) {
        $error = "Input must four integer ";
    } else {
        $md5 = hash('md5', $code);
    }
}
?>
<!DOCTYPE html>
<head><title>Gazmor Abdiu PIN Code</title></head>
<body>
<h1>MD5 PIN Maker</h1>
<?php
if ( $error !== false ) {
...
?>
<p>Please enter a four digit for encoding.</p>
<form>
